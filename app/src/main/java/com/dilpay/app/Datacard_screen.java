...
public class Datacard_screen extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!CheckNetworkStatus.getConnectivityStatusString(Datacard_screen.this)) {
            Toast.makeText(Datacard_screen.this, "No internet connection", Toast.LENGTH_SHORT).show();
            finish();
        }
        ...
        operatorName.add("Tata Photon Whiz");
        operatorName.add("Tata Photon+");
        operatorName.add("MTS Mblaze");
        operatorName.add("MTS Mbrowse");

        operatorCode.add("RNET3");
        operatorCode.add("RNETP");
        operatorCode.add("TPW");
        operatorCode.add("TPP");
        operatorCode.add("MTM");
        operatorCode.add("MTBR");
...
//                getResponse(number, year + "" + rand_int1, username, password, amount, operator_code, circle_code);
                Intent intent =new Intent(Datacard_screen.this, SecurityPin.class);
    ...
    private void getResponse(String number, String orderid, String username, String password, String amount, String operator_code, String circle_code) {
        ...
        try {
            dataObj.put("username", new userDetails(Datacard_screen.this).getNumber());
...
//                          i.putExtra("opid",obj.getInt("opid"));
                            ...
                            Toast.makeText(Datacard_screen.this, "Enter valid details!", Toast.LENGTH_SHORT).show();
                        }
                    }else{
                        Toast.makeText(Datacard_screen.this, obj.getString("Data"), Toast.LENGTH_SHORT).show();
                        finish();
                    }
                } catch (Exception e) {
